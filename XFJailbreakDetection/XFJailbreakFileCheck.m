...
+(BOOL)isJailbreakFileExist {
    ...
    for (NSString *jbFile in jbPatternFile) {
        ...
        if ([fileManager fileExistsAtPath:jbFile]) {
            [Util appendTextToOutput:[NSString stringWithFormat:@"NSFilemanager: %@", jbFile]];
            check = YES;
        }
        ...
        if (dirPoint != NULL) {
            [Util appendTextToOutput:[NSString stringWithFormat:@"opendir: %@ - %p", jbFile, dirPoint]];
            check = YES;
        }

        //syscall - SYS_access
        if(syscall(SYS_access, jbFile2, F_OK) == 0) {
            [Util appendTextToOutput:[NSString stringWithFormat:@"Syscall SYS_access: %@", jbFile]];
            check = YES;
        }
    ...
    #endif
        if (flag != ENOENT ) {
            [Util appendTextToOutput:[NSString stringWithFormat:@"SVC #0x80 SYS_syscall - SYS_access: %s", jbFile2]];
            check = YES;
        }
    ...
    #endif
        if (flag != ENOENT ) {
            [Util appendTextToOutput:[NSString stringWithFormat:@"SVC #0x80 SYS_access: %s", jbFile2]];
            check = YES;
        }
    ...
    #endif
        if (flag != ENOENT ) {
            [Util appendTextToOutput:[NSString stringWithFormat:@"SVC #0x80 SYS_lstat64: %s", jbFile2]];
            check = YES;
        }
    ...
    #endif
        if (flag != ENOENT ) {
            [Util appendTextToOutput:[NSString stringWithFormat:@"SVC #0x80 SYS_stat64: %s", jbFile2]];
            check = YES;
        }
    ...
    #endif
        if (flag != ENOENT ) {
            [Util appendTextToOutput:[NSString stringWithFormat:@"SVC #0x80 SYS_statfs64: %s", jbFile2]];
            check = YES;
        }
    ...
    #endif
        if(flag == 1) {
            [Util appendTextToOutput:[NSString stringWithFormat:@"SVC #0x80 SYS_open: %s", jbFile2]];
            check = YES;
        }
    }
    return check;
