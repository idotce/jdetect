...
+(BOOL)isJailbreakInjectExist {
    ...
    for (NSString *jbSymbol in jbPatternSymbol) {
        ...
        if(dlpoint != NULL) {
            [Util appendTextToOutput:[NSString stringWithFormat:@"dlsym: %s - %p", jbSymbol2, dlpoint]];
            check = YES;
        }
    }
    ...
    for(uint32_t i = 0; i < count; i++) {
        ...
        for (NSString *jbDyld in jbPatternDyld) {
            ...
            if([detectedDyld containsString:jbDyld]) {
                [Util appendTextToOutput:[NSString stringWithFormat:@"dyld: %@", detectedDyld]];
                check = YES;
                break;
            }
        }
                        ...
                        if (sc->cmd == LC_ID_DYLIB) {
                            ...
                            for (NSString *jbDyld in jbPatternDyld) {
                                if([[NSString stringWithUTF8String:detectedDyld] containsString:jbDyld]) {
                                    [Util appendTextToOutput:[NSString stringWithFormat:@"dyld2: %s", detectedDyld]];
                                    check = YES;
                                    break;
                                }
                            }
        ...
        while (*env) {
            for (NSString *jbEnv in jbPatternEnv) {
                if([[NSString stringWithUTF8String:*env] containsString:jbEnv]) {
                    [Util appendTextToOutput:[NSString stringWithFormat:@"env: %s", *env]];
                    check = YES;
                }
            }
            env++;
    ...
    for(int i=0; environ[i]; i++)
    {
        for (NSString *jbEnv in jbPatternEnv) {
            if([[NSString stringWithUTF8String:environ[i]] containsString:jbEnv]) {
                [Util appendTextToOutput:[NSString stringWithFormat:@"env2 <%d>: %s", i, environ[i]]];
                check = YES;
            }
        }
    }
